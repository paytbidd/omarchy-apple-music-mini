...
- **System font** is off by default. Enable it in the Apple Music bar widget settings (“Use Omarchy system font”), or `apple-music-mini font on`. That uses the current Omarchy font at ~92% of Apple’s 13px SF Pro metrics, plus a stronger sidebar hover. `omarchy font set` updates the family for the next launch.
